...
async function run() {
  ...
  emitProgress({ percent: 2, message: "Meta ads intelligence discovery started", totalFound: 0 });

  const results = await discoverViaGoogleMaps({
    keyword: `${businessType} facebook ads`,
    location,
    maxItems,
    category: businessType,
    snippetPrefix: "Ads Intel: Meta paid-intent proxy via Google Maps",
    transformRecord: (record) => ({
      ...record,
      website: "",
      network: "meta",
      model: "meta_paid_intent_proxy",
      confidence: 0.62,
    }),
    onProgress: ({ processedQueries, totalQueries, totalListings, previewListings }) => {
      ...
      emitProgress({
        percent,
        message: `Meta ads-intel fallback ${Math.min(discovered, maxItems)}/${maxItems}`,
  ...
  emitProgress({
    percent: 100,
    message: `Meta ads intelligence finished with ${results.length} rows`,
...
run().catch((error) => {
  ...
  console.error(`[ads-meta-runner] ${message}`);
  process.exit(1);
});
