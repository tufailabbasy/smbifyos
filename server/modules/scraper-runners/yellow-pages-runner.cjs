...
async function run() {
  ...
  const maxItems = normalizePositiveInteger(payload.maxItems ?? payload.maxPages, 20, 5000);

  if (!businessType || !location) {
    throw new Error("businessType and location are required");
  }

  emitProgress({ percent: 2, message: "Yellow Pages discovery started", totalFound: 0 });
  ...
  const results = await discoverViaGoogleMaps({
    ...
    snippetPrefix: "Directory mode: Yellow Pages fallback via Google Maps",
    onProgress: ({ processedQueries, totalQueries, totalListings, previewListings }) => {
      ...
      emitProgress({
        percent,
        message: `Yellow Pages fallback discovery ${Math.min(discovered, maxItems)}/${maxItems}`,
  ...
  emitProgress({
    percent: 100,
    message: `Yellow Pages discovery finished with ${results.length} rows`,
...
run().catch((error) => {
  ...
  console.error(`[yellow-pages-runner] ${message}`);
  process.exit(1);
});
