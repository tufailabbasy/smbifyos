...
async function run() {
  ...
  emitProgress({ percent: 2, message: "BBB discovery started", totalFound: 0 });
  ...
  const results = await discoverViaGoogleMaps({
    ...
    snippetPrefix: "Directory mode: BBB fallback via Google Maps",
    onProgress: ({ processedQueries, totalQueries, totalListings, previewListings }) => {
      ...
      emitProgress({
        percent,
        message: `BBB fallback discovery ${Math.min(discovered, maxItems)}/${maxItems}`,
  ...
  emitProgress({
    percent: 100,
    message: `BBB discovery finished with ${results.length} rows`,
...
run().catch((error) => {
  ...
  console.error(`[bbb-runner] ${message}`);
  process.exit(1);
});
