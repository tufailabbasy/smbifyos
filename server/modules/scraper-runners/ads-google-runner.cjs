...
async function run() {
  ...
  emitProgress({ percent: 2, message: "Google ads intelligence discovery started", totalFound: 0 });

  const results = await discoverViaGoogleMaps({
    keyword: `${businessType} advertising`,
    location,
    maxItems,
    category: businessType,
    snippetPrefix: "Ads Intel: Google paid-intent proxy via Google Maps",
    transformRecord: (record) => ({
      ...record,
      website: "",
      network: "google",
      model: "google_maps_paid_intent_proxy",
      confidence: 0.67,
    }),
    onProgress: ({ processedQueries, totalQueries, totalListings, previewListings }) => {
      ...
      emitProgress({
        percent,
        message: `Google ads-intel fallback ${Math.min(discovered, maxItems)}/${maxItems}`,
  ...
  emitProgress({
    percent: 100,
    message: `Google ads intelligence finished with ${results.length} rows`,
...
run().catch((error) => {
  ...
  console.error(`[ads-google-runner] ${message}`);
  process.exit(1);
});
