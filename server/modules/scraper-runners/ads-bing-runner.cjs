...
async function run() {
  ...
  emitProgress({ percent: 2, message: "Microsoft/Bing ads intelligence discovery started", totalFound: 0 });

  const results = await discoverViaGoogleMaps({
    keyword: `${businessType} microsoft ads`,
    location,
    maxItems,
    category: businessType,
    snippetPrefix: "Ads Intel: Microsoft/Bing paid-intent proxy via Google Maps",
    transformRecord: (record) => ({
      ...record,
      website: "",
      network: "microsoft_bing",
      model: "bing_paid_intent_proxy",
      confidence: 0.57,
    }),
    onProgress: ({ processedQueries, totalQueries, totalListings, previewListings }) => {
      ...
      emitProgress({
        percent,
        message: `Microsoft/Bing ads-intel fallback ${Math.min(discovered, maxItems)}/${maxItems}`,
  ...
  emitProgress({
    percent: 100,
    message: `Microsoft/Bing ads intelligence finished with ${results.length} rows`,
...
run().catch((error) => {
  ...
  console.error(`[ads-bing-runner] ${message}`);
  process.exit(1);
});